...
function FilmsForm(props){
    ...
    const [favorites, setFavorites] = useState(props.film ? props.film.favorites : 0);
    const [date, setDate] = useState(props.film ? dayjs(props.film.date).format('YYYY-MM-DD') : dayjs().format('YYYY-MM-DD'));
    const [rating, setRating] = useState(props.film ? props.film.rating : null);
    ...
    const handleSubmit = (event) => {
        ...
        if(Object.keys(validationErrors).length === 0){
          setWaiting(true);
            if(props.mode === 'edit'){
                API.updateMovie(movie,props.film.id)
                .then(() => {if(props.filter === 'All') navigate('../');
                              else navigate(`../${props.filter}`);
              });
               } else {
                  API.addMovie(movie)
                  .then(() => {if(props.filter === 'All') navigate('../');
    ...
    const validateForm = () => {
        ...
        if(!dayjs(date).isValid() && rating!==null){
    ...
    return(
            <Form onSubmit={handleSubmit}>
                <Row className="text-light text-center">
                    ...
                    <Col>
                        {props.mode==='add' && <Button className='btn-success mx-1' type='submit' disabled={waiting}><i className='bi bi-check'></i></Button>}
                        {props.mode==='edit' && <Button className='btn btn-primary mx-1' type='submit' disabled={waiting}><i className='bi bi-pencil-square'></i></Button>}
...
function FavoriteCheckboxNew (props) {
    return(
        <>
            ...
            <input type="checkbox" checked={props.favorites} onChange={()=>props.setFavorites(props.favorites ? 0 : 1)}/>
