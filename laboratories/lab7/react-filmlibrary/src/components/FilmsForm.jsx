...
function FilmsForm(props){
    ...
    const handleSubmit = (event) => {
        ...
        if(Object.keys(validationErrors).length === 0){
            if(props.mode === 'edit'){
                props.updateMovie({id: props.film.id, ...movie});
               } else {
    ...
    return(
            <Form onSubmit={handleSubmit}>
                <Row className="text-light text-center">
                    ...
                    <Col>
                        {props.mode==='add' && <Button variant='success' className='mx-1' type='Submit'><i className='bi bi-check'></i></Button>}
                        {props.mode==='edit' && <Button variant='primary' className='mx-1' type='Submit'><i className='bi bi-pencil-square'></i></Button>}
                        <Button variant='danger' onClick={props.cancel}><i className='bi bi-x'></i></Button>
                    </Col>
                </Row>
            </Form>  
    );
